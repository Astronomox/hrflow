...
import { Upload, FolderOpen, Trash2, Download, FileText, Loader2, CloudUpload } from "lucide-react";
...
function FileGrid({ scope }: { scope: "mine" | "department" | "all" }) {
  ...
  return (
    <>
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {files.map((f: { id: string; name: string; mimeType: string; size: number; createdAt: string; url: string; uploader: { user: { name: string } } }) => (
            ...
            <CardContent className="p-4">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">{f.uploader.user.name}</p>
                </div>
              </div>
              <div className="flex items-center gap-1.5 mt-3 pt-3 border-t border-border/40 opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                <Button size="sm" variant="ghost" className="h-7 w-7 p-0 text-destructive hover:text-destructive hover:bg-destructive/10 shrink-0" onClick={() => setDeleteId(f.id)}>
...
export default function FilesPage() {
  ...
  const onDrop = useCallback((accepted: File[]) => {
    accepted.forEach(file => uploadFile.mutate({ file }));
  }, [uploadFile]);
      ...
      >
        ...
        <div className="flex flex-col items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">PDF, images, Word, CSV — up to 10MB per file</p>
          </div>
        </div>
      </div>
